...
import '../../compiler/js_split_facade.dart';
import 'ast.dart'
    show
        AST,
        ASTWithSource,
...
class Parser {
  ...
  Parser(
    this._lexer, {
    this.supportNewPipeSyntax = true,
  });
  ...
  ASTWithSource parseAction(
    ...
    var tokens = _tokenizeOrThrow(_stripComments(input), input, location);
    var ast = _ParseAST(
      input,
      location,
      tokens,
      true,
      exports,
      supportNewPipeSyntax: supportNewPipeSyntax,
      supportListLiterals: CompileContext.current.allowListLiterals(location),
    ).parse();
...
class _ParseAST {
  ...
  final bool supportListLiterals;
  final bool supportNewPipeSyntax;
  ...
  _ParseAST(
    ...
    this.parseAction,
    List<CompileIdentifierMetadata> exports, {
    @required this.supportNewPipeSyntax,
    @required this.supportListLiterals,
  }) {
  ...
  @alwaysThrows
  void throwForLiteralList() {
    throw ParseException(
      'UNSUPPORTED: List literals are no longer supported in the template.\n'
      'Move code that constructs or maintains List instances inside of your '
      '@Component-annotated Dart class, or prefer syntax such as '
      '[class.active]="isActive" over [ngClass]="[\'active\']".',
      input,
      location,
    );
  }
