...
void main() {
  group('integration tests', () {
    setUp(() {
      ...
      template.append(new DivElement());
      beforeEachProviders(() => [provide(ANCHOR_ELEMENT, useValue: template)]);
    });
    group('react to record changes', () {
      ...
      test(
        ...
        inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
            ...
            expect(elm.attributes['foo'], 'bar');
            fixture.debugElement.componentInstance.ctxProp = null;
            fixture.detectChanges();
            expect(elm.attributes.containsKey('foo'), isFalse);
            completer.done();
          });
        });
      });
      test('should remove style when when style expression evaluates to null',
        ...
        return inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
              .overrideView(MyComp,
                  new View(template: '<div [style.height.px]="ctxProp"></div>'))
              .createAsync(MyComp)
              .then((fixture) {
            fixture.debugElement.componentInstance.ctxProp = '10';
            fixture.detectChanges();
            Element elm = fixture.debugElement.children[0].nativeElement;
            expect(elm.style.height, '10px');
            fixture.debugElement.componentInstance.ctxProp = null;
            fixture.detectChanges();
            expect(elm.style.height, '');
            completer.done();
          });
        });
      });
      test(
          'should consume binding to property names where attr '
          'name and property name do not match', () async {
        return inject([TestComponentBuilder, AsyncTestCompleter],
            (TestComponentBuilder tcb, AsyncTestCompleter completer) {
          tcb
              .overrideView(MyComp,
      ...
      test('should consume binding to innerHtml', () async {
        return inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
            ...
            fixture.detectChanges();
            Element elm = fixture.debugElement.children[0].nativeElement;
            expect(elm.innerHtml, 'Some <span>HTML</span>');
            fixture.debugElement.componentInstance.ctxProp =
                'Some other <div>HTML</div>';
            fixture.detectChanges();
            expect(elm.innerHtml, 'Some other <div>HTML</div>');
            completer.done();
          });
        });
      });
      test('should consume binding to className using class alias', () async {
        return inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
              ...
              .createAsync(MyComp)
              .then((fixture) {
            Element nativeEl = fixture.debugElement.children[0].nativeElement;
            fixture.debugElement.componentInstance.ctxProp = 'foo bar';
            fixture.detectChanges();
            expect(nativeEl.classes.contains('foo'), isTrue);
            expect(nativeEl.classes.contains('bar'), isTrue);
            expect(nativeEl.classes.contains('initial'), isFalse);
            completer.done();
          });
        });
      });
      ...
      test('should support template directives via `<template>` elements.',
        ...
        return inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
            ...
            var childNodesOfWrapper = elm.childNodes;
            // 1 template + 2 copies.
        ...
        return inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
              .overrideView(
                  MyComp,
                  new View(
                      template: '<div *ngIf="ctxBoolProp">'
                          '<template some-viewport let-greeting="someTmpl">'
                          '<span>{{greeting}}</span></template></div>',
        ...
        return inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
            ...
            var childNodesOfWrapper =
                fixture.debugElement.nativeElement.childNodes;

            expect(childNodesOfWrapper, hasLength(1));
            expect(childNodesOfWrapper[0] is Comment, isTrue);
            completer.done();
          });
        });
      });
        ...
        return inject([TestComponentBuilder, AsyncTestCompleter],
          ...
          tcb
              .overrideView(
                  ...
                  new View(
                      template:
                          '<copy-me template="some-viewport: let greeting=some-tmpl">'
                          '{{greeting}}</copy-me>',
                      directives: [SomeViewport]))
              .createAsync(MyComp)
              .then((fixture) {
              ...
              .overrideView(
                  ...
                  new View(
                      template: '<some-directive><toolbar>'
                          '<template toolbarpart let-toolbarProp="toolbarProp">{{ctxProp}},'
                          '{{toolbarProp}},<cmp-with-host></cmp-with-host>'
                          '</template></toolbar></some-directive>',
                      directives: [
                        SomeDirective,
                        CompWithHost,
                        ToolbarComponent,
          ...
          return inject([TestComponentBuilder, AsyncTestCompleter],
            ...
            tcb
                .overrideView(
                    ...
                    new View(
                        template:
                            '<template [ngIf]="true">{{alice.ctxProp}}</template>'
                            '|{{alice.ctxProp}}|<child-cmp ref-alice></child-cmp>',
        ...
        test('should allow to use variables in a for loop', () async {
          return inject([TestComponentBuilder, AsyncTestCompleter],
            ...
            tcb
              ...
              Element elm = fixture.debugElement.nativeElement;
              expect(elm.childNodes[2], hasTextContent('1-hello'));
              completer.done();
            });
          });
        });
        ...
        test('should support updating host element via hostAttributes',
          ...
          return inject([TestComponentBuilder, AsyncTestCompleter],
            ...
            tcb
              ...
              fixture.detectChanges();
              Element elm = fixture.debugElement.children[0].nativeElement;
              expect(elm.attributes['role'], 'button');
              completer.done();
            });
          });
        });
        ...
        test('should support preventing default on render events', () async {
          return inject([TestComponentBuilder, AsyncTestCompleter],
            ...
            tcb
                ...
                .createAsync(MyComp)
                .then((fixture) {
              var dispatchedEvent = new MouseEvent('click', canBubble: true);
              var dispatchedEvent2 = new MouseEvent('click', canBubble: true);
              Element elm1 = fixture.debugElement.children[0].nativeElement;
              elm1.dispatchEvent(dispatchedEvent);
              Element elm2 = fixture.debugElement.children[1].nativeElement;
              elm2.dispatchEvent(dispatchedEvent2);
              expect(dispatchedEvent.defaultPrevented, isTrue);
              expect(dispatchedEvent2.defaultPrevented, isFalse);
              expect((elm1 as InputElement).checked, isFalse);
              expect((elm2 as InputElement).checked, isTrue);
              completer.done();
            });
          });
        });
        ...
        test('should remove script tags from templates', () async {
          return inject([TestComponentBuilder, AsyncTestCompleter],
            ...
            <div>before<script>alert("Ooops");</script><span>inside</span>after</div>'''))
                ...
                .then((fixture) {
              expect(
                  fixture.debugElement.nativeElement.querySelectorAll('script'),
                  hasLength(0));
              completer.done();
            });
          });
      ...
      group('error handling', () {
        ...
        group('Property bindings', () {
            ...
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb.createAsync(MyComp).catchError((e) {
                expect(
                    ...
                    'line 1, column 6 of MyComp: ParseErrorLevel.FATAL: Can\'t bind to \'unknown\' since it isn\'t a known native property or known directive. Please fix typo or add to directives list.\n'
                    'unknown="{{ctxProp}}"\n'
                    '^^^^^^^^^^^^^^^^^^^^^');
                completer.done();
                return null;
          ...
          test(
              'should not throw for property binding to a non-existing '
              'property when there is a matching directive property', () async {
            return inject([TestComponentBuilder, AsyncTestCompleter],
                (TestComponentBuilder tcb, AsyncTestCompleter completer) {
              tcb
                  .overrideView(
          ...
          test(
            ...
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                fixture.detectChanges();
                Element elm = fixture.debugElement.nativeElement;
                var titleElm = elm.querySelector('span');
                expect(titleElm.title, 'TITLE');
                completer.done();
              });
            });
          });
          ...
          test('should reflect property values as attributes', () async {
            ...
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                fixture.detectChanges();
                Element elm = fixture.debugElement.nativeElement;
                expect(elm.innerHtml, contains('ng-reflect-dirprop="hello"'));
                completer.done();
              });
            });
          });
          test('should reflect property values on template comments', () async {
            ...
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                fixture.detectChanges();
                Element elm = fixture.debugElement.nativeElement;
                expect(elm.innerHtml, contains('"ng-reflect-ngIf": "true"'));
                completer.done();
              });
            });
          });
          ...
          test('should support host binding decorators', () async {
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                fixture.detectChanges();
                Element elm = fixture.debugElement.children[0].nativeElement;
                expect(elm.outerHtml, contains('my-attr="aaa"'));
                completer.done();
              });
            });
          });
          ...
          test('should support host listener decorators', () async {
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                native.dispatchEvent(new MouseEvent('click', canBubble: true));
          ...
          test('should support svg elements', () async {
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                expect(js_util.getProperty((svg as dynamic), 'namespaceURI'),
                    'http://www.w3.org/2000/svg');
                expect(js_util.getProperty((use as dynamic), 'namespaceURI'),
                    'http://www.w3.org/2000/svg');
                // For Dart where '_Attr' has no instance getter 'namespaceURI'
                expect(use.outerHtml, contains('xmlns:xlink'));
                completer.done();
              });
            });
          });
          test('should support svg elements', () async {
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                var el = fixture.debugElement.nativeElement;
                var svg = el.childNodes[0];
                var foreignObject = svg.childNodes[0];
                var div = foreignObject.childNodes[0];
                var p = div.childNodes[0];
                expect(js_util.getProperty(svg, 'namespaceURI'),
                    'http://www.w3.org/2000/svg');
                expect(js_util.getProperty(foreignObject, 'namespaceURI'),
                    'http://www.w3.org/2000/svg');
                expect(js_util.getProperty(div, 'namespaceURI'),
                    'http://www.w3.org/1999/xhtml');
                expect(js_util.getProperty(p, 'namespaceURI'),
                    'http://www.w3.org/1999/xhtml');
                completer.done();
              });
            });
          ...
          test('should support binding to attributes with namespace', () async {
            return inject([TestComponentBuilder, AsyncTestCompleter],
              ...
              tcb
                ...
                Element elm = fixture.debugElement.nativeElement;
                var useEl = elm.firstChild as Element;
                cmp.value = '#id';
                fixture.detectChanges();
                expect(
                    useEl.getAttributeNS(
...
class DuplicateDir {
  DuplicateDir(ElementRef elRef) {
    Element elm = elRef.nativeElement;
    elm.text = '${elm.text}noduplicate';
  }
}
...
class OtherDuplicateDir {
  OtherDuplicateDir(ElementRef elRef) {
    Element elm = elRef.nativeElement;
    elm.text = '${elm.text}othernoduplicate';
  }
}
